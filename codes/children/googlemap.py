...
'''
df = pd.DataFrame({'name':[],'address':[],'lat':[],'lng':[],'place_id':[],'rating':[],'rating_total':[]})
...
with open(ID_CSV_PATH, mode='w', newline='', encoding='shift-jis', errors='ignore') as f:
...
os.makedirs(os.path.join(BASE_DIR,'store'), exist_ok=True)
with open(Detail_CSV_PATH, mode='w', newline='', encoding='shift-jis', errors='ignore') as f:
