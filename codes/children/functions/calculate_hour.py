        ...
        for e in week.split(','): 
            ...
            if 'Closed' in e:
                ...
                if ',' in e:
                    ...
                    start_time = e.replace(' ','').replace(str(l+':'),'').split('–')[0]
                    end_time = e.replace(' ','').replace(str(l+':'),'').split('–')[1]
                    if 'AM' in start_time:
                        Hour_start = float(start_time.replace('AM','').split(':')[0])
                        Minitues_start = float(start_time.replace('AM','').split(':')[1])/60
                    elif 'PM' in start_time:
                        Hour_start = float(start_time.replace('PM','').split(':')[0])+12
                        Minitues_start = float(start_time.replace('PM','').split(':')[1])/60
                    ...
                    if 'AM' in end_time:
                        ...
                        Minitues_end = float(end_time.replace('AM','').split(':')[1])/60
                    elif 'PM' in end_time:
                        Hour_end = float(end_time.replace('PM','').split(':')[0])+12
                        Minitues_end = float(end_time.replace('PM','').split(':')[1])/60
                    business_hours[num] = (Hour_end + Minitues_end) -(Hour_start + Minitues_start)+business_hours[num]
