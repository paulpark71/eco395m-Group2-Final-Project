...
'''
Tokenized_text=tokenized_without_stopwords(dataset, Stopwords_Path, Additional_Stopwords_Path )
...
with open(Count_CSV_PATH, mode='w', newline='', encoding='shift-jis', errors='ignore') as f:
    all_count_data.to_csv(Count_CSV_PATH,header=True)

with open(Count_Each_CSV_PATH, mode='w', newline='', encoding='shift-jis', errors='ignore') as f:
...
count_integrated_data=pd.merge(integrated_data,count_data,how='inner', on='place_id')

with open(Count_integrated_CSV_PATH, mode='w', newline='', encoding='shift-jis', errors='ignore') as f:
